    ...
    for (i = n - 1; i >= 1; i--)
    {
        ...
        for (j = i - 1; j >= 0; j--) {
            if (rc[max_idx].Key < rc[j].Key) {
                ...
                setWhiteDiv(mypos.posx + (mypos.width + 5) * max_idx, mypos.posy, mypos.width, rc[max_idx].Key);
                createRedDiv(mypos.posx + (mypos.width + 5) * i, mypos.posy, mypos.width, rc[i].Key);
                swapbuffers();
                max_idx = j;
            }
                ...
                createRedDiv(mypos.posx + (mypos.width + 5) * i, mypos.posy, mypos.width, rc[i].Key);
                if (max_idx != i) {
                    setYellowDiv(mypos.posx + (mypos.width + 5) * max_idx, mypos.posy, mypos.width, rc[max_idx].Key);
                }
        ...
        if (max_idx != i) {
            ...
            resetScreen(rc);
            createRedDiv(mypos.posx + (mypos.width + 5) * i, mypos.posy, mypos.width, rc[i].Key);
            setLightGreenDiv(mypos.posx + (mypos.width + 5) * max_idx, mypos.posy, mypos.width, rc[max_idx].Key);
            swapbuffers();
            Sleep(1000);
            animateSwap2Div(rc, max_idx, i);
            myswap(rc[i], rc[max_idx]);
            swapbuffers();
